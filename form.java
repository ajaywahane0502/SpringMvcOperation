        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(ServletFileUpload.isMultipartContent(request))
            {
                ...
                try
                {
                    ...
                    for(FileItem item : multiparts)
                    {
                        ...
                        else
                        {
                            ...
                            if(item.getFieldName().equalsIgnoreCase("securityquestion"))
                            {
                                question=item.getString();
                            }
                            if(item.getFieldName().equalsIgnoreCase("answer"))
                            {
                ...
                try
                {
                    ...
                    String sql="insert into form values('"+username+"','"+email+"','"+password+"','"+question+"','"+answer+"','"+filename+"','"+path+"')";
                    pst=con.prepareStatement(sql);
                    int i=pst.executeUpdate();
                    if(i>0)
                    {
                        response.sendRedirect("DisplayImg.jsp?filename="+filename+"&username="+username);
                        //RequestDispatcher rd=request.getRequestDispatcher("DisplayImg.jsp?filename="+filename);
